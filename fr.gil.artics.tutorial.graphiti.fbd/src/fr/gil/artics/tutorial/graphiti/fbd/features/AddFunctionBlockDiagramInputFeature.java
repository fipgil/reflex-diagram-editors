...
public class AddFunctionBlockDiagramInputFeature extends AbstractAddShapeFeature {

	private static final int ANCHOR_SIZE = 20;
	private static final int MARGIN_SIZE = 10;

	public AddFunctionBlockDiagramInputFeature(IFeatureProvider fp) {
		super(fp);
	}

	public boolean canAdd(IAddContext context) {
		// check if user wants to add an input
		if (context.getNewObject() instanceof FunctionBlockDiagramInput) {
	...
	public PictogramElement add(IAddContext context) {
		...
		FunctionBlockDiagramInput input = (FunctionBlockDiagramInput) context.getNewObject();
		Diagram targetDiagram = (Diagram) context.getTargetContainer();

		getFeatureProvider().link(targetDiagram, input.getParent());
		...
		final Text text = gaService.createPlainText(shape, input.getName());
		...
		gaService.setLocationAndSize(roundedRectangle, 0, 0, width + ANCHOR_SIZE / 2 + 2 * MARGIN_SIZE,
				height + 2 * MARGIN_SIZE);
		gaService.setLocationAndSize(text, MARGIN_SIZE, MARGIN_SIZE, width, height);
